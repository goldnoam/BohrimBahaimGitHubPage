...
const InfoCard: React.FC<InfoCardProps> = ({ item }) => {
  return (
    <div className="bg-white rounded-xl shadow-md p-5 md:p-6 border-t-4 border-indigo-500 hover:shadow-lg transition-all duration-300 h-full relative group">
      <div className="absolute top-2 left-2 flex flex-col gap-2 md:opacity-60 group-hover:opacity-100 transition-opacity">
        {/* Slightly larger touch area for mobile */}
        <ShareButton title={item.title} text={item.description} className="w-10 h-10 md:w-9 md:h-9" />
      </div>
      <div className="text-3xl md:text-4xl mb-4 text-center">{item.icon}</div>
