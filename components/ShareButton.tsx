...
const ShareButton: React.FC<ShareButtonProps> = ({ title, text, className = "" }) => {
  ...
  return (
    <button 
      onClick={handleShare}
      title="שתף מידע זה"
      className={`p-2 rounded-full bg-white/80 hover:bg-white text-slate-400 hover:text-indigo-600 shadow-sm transition-all flex items-center justify-center border border-slate-100 ${className}`}
    >
